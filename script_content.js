...
let localS = JSON.parse(localStorage.getItem('lifeOS_v2_local')) || {
  budgets: [
    { cat:'Food & Dining', limit:5000, color:'terracotta' },
    { cat:'Transport', limit:2000, color:'sky' },
    { cat:'Entertainment', limit:2000, color:'rose' },
    { cat:'Shopping', limit:4000, color:'gold' },
  ],
...
function saveLocal() {
  localStorage.setItem('lifeOS_v2_local', JSON.stringify({
    budgets: S.budgets,
    calYear: S.calYear,
    calMonth: S.calMonth,
...
async function loadData() {
  try {
    const [txns, goals, tasks, habits, events] = await Promise.all([
    ...
    // Map habits logs back to week logic needed for UI
